...
class MainView(View):
    def get(self, request):
        sklep = Sklep.objects.filter(serwis_zew=False).filter(is_active=True)
...
class UserLoginView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid() and reCapForm.is_valid():
...
class PhoneUpdateView(PermissionRequiredMixin, UpdateView):
    permission_required = 'miktel.del_telefon'
    raise_exception = True
    model = Telefon
    fields = '__all__'
    template_name_suffix = ('_update_form')
    ...
    def get_success_url(self):
        self.object.is_in_promo()
        self.object.save()
        return "/miktel/telefony_magazyn/"
...
class TelefonyMagazynView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        shops = sklep = Sklep.objects.filter(serwis_zew=False).filter(is_active=True)
...
class CheckingDocumentView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        pracownik = request.user
        sklepy = pracownik.sklep.all()
        umowa = UmowaKomisowaNew.objects.first()
...
class ListaSerwisowMagazynView(PermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        serwisy_dostepne = DodajSerwis.objects.filter(
            archiwum=False).order_by('-data')
...
class ArchiwumSerwisView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        serwisy = DodajSerwis.objects.filter(
            archiwum=True).order_by('-data_wydania')
        page = request.GET.get('page')
        global page_records
        paginator = Paginator(serwisy, page_records)
        serwisy_pagi = paginator.get_page(page)
...
@method_decorator(login_required, name='dispatch')
class GetExcelServices(PermissionRequiredMixin, View):
    permission_required = 'miktel.view_dodajserwis'
    ...
    def get(self, request):
        ...
        services = []
        for el in serwisy:
            if el.data != None and el.data_wydania != None:
                el.data = str(el.data.date()) + " " + str(el.data.time())
                el.data_wydania = str(el.data_wydania.date()) + " " + str(
                    el.data_wydania.time())
                service = []
                service.append(el.id)
                service.append(el.data)
                service.append(el.sklep.nazwa)
                service.append(el.pracownik)
                service.append(el.marka.nazwa)
                service.append(el.model)
                service.append(el.cena_zgoda)
                service.append(el.koszt)
                service.append(el.data_wydania)
                service.append(el.serwisant)
                service.append(el.usluga.nazwa)
                service.append(round(el.cena_zgoda - el.koszt, 2))
        ...
        df = pd.DataFrame(services,
                          columns=[
                              'id', 'data', 'sklep', 'pracownik', 'marka',
                              'model', 'cena_zgoda', 'koszt', 'data_wydania',
                              'serwisant', 'usługa', 'zysk'
                          ])
        with pd.ExcelWriter("static/media/services.xlsx") as writer:
            df.to_excel(writer)
        return TemplateResponse(request, 'get_excel.html')
...
class ReklamacjaSerwisView(PermissionRequiredMixin, View):
    ...
    def get(self, request, pk):
        serwis = DodajSerwis.objects.get(pk=pk)
        serwis.sklep = request.user.sklep_dzisiaj
...
class ListaCzesciView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        czesc = Czesc.objects.filter(sklep=lokal).filter(ilosc__gt=0)
        page = request.GET.get('page')
        global page_records
        lokale = Sklep.objects.all()
        ...
        if zero == "on":
            czesc = Czesc.objects.filter(sklep=lokal).filter(ilosc__gte=0)
        if search:
            czesc = Czesc.objects.all()
            q_object = reduce(and_, (Q(marka__nazwa__contains=search)
                                     | Q(typ__nazwa__contains=search)
                                     | Q(nazwa__contains=search)
                                     for search in search))
            czesc = czesc.filter(q_object)
            if zero == "on":
                czesc = czesc.filter(ilosc__gte=0)
            else:
                czesc = czesc.filter(ilosc__gt=0)
...
class ListaCzesciAllView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        global page_records
        lokale = Sklep.objects.all()
        czesc = Czesc.objects.filter(ilosc__gt=0)
        search = request.GET.get('search')
        zero = request.GET.get('zero')
        if zero == "on":
            czesc = Czesc.objects.filter(ilosc__gte=0)
        if search:
            czesc = Czesc.objects.all()
            q_object = reduce(and_, (Q(marka__nazwa__contains=search)
                                     | Q(typ__nazwa__contains=search)
...
class GetServiceForItems(PermissionRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        if zero != 'on':
            items = items.filter(ilosc__gt=0)
        page = request.GET.get('page')
    ...
    def post(self, request, pk):
        service = DodajSerwis.objects.get(pk=pk)
        marka = service.marka
        search = request.POST.getlist('szukaj')
        zero = request.POST.getlist('zero')
        q_object = reduce(and_, (Q(marka__nazwa__contains=search)
                                 | Q(typ__nazwa__contains=search)
                                 | Q(nazwa__contains=search)
                                 for search in search))
...
class DashboardView(PermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        miesiac_nazwa = MIESIACE[int(miesiac) - 1][1]
        ctx = {
...
class SzczegolySerwisySerwisantaView(PermissionRequiredMixin, View):
    ...
    def get(self, request, pk, miesiac, rok, sklep):
        rok = rok
        # miesiac = datetime.now().month
        ...
        sklep = Sklep.objects.get(pk=sklep)
        usluga_akc = Usluga.objects.get(akcesoria=True)
        akcesoria = PremiaJob.objects.filter(
            data__year=rok,
            data__month=miesiac).filter(pracownik=pracownik).filter(
                usluga=usluga_akc).filter(sklep=sklep)
        usluga_serwis = Usluga.objects.filter(akcesoria=False).filter(
            sklep=sklep)
        ...
        uslugi = PremiaJob.objects.filter(
            data__year=rok,
            data__month=miesiac).filter(pracownik=pracownik).filter(
                usluga__in=usluga_serwis).filter(sklep=sklep).order_by('-id')
        inne = InnePracePremiowane.objects.filter(
            data__year=rok,
            data__month=miesiac).filter(pracownik=pracownik).order_by('-id')
        zysk = []
    ...
    def post(self, request, pk, miesiac, rok, sklep):
        pracownik = MyUser.objects.get(pk=pk)
        miesiac_filter = request.POST.get('miesiac')
        rok_filter = request.POST.get('rok')
        rok = rok_filter
        pracownik = request.user
        data = MIESIACE[int(miesiac_filter) - 1][1]
...
class AddProductView(PermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            product.sale = sale
            if sale == "1":
                set_price_promo = int(price) * 0.9
                product.price_promo = round(set_price_promo, 2)
                product.is_sale = True
                print('chuj')
            if sale == "2":
                product.price_promo = price * 0.8
                product.price_promo = round(product.price_promo, 2)
                product.is_sale = True
            if sale == "3":
                product.price_promo = price * 0.7
                product.price_promo = round(product.price_promo, 2)
                product.is_sale = True
            if sale == "4":
                product.price_promo = price * 0.6
                product.price_promo = round(product.price_promo, 2)
                product.is_sale = True
            if sale == "5":
                product.price_promo = price * 0.5
                product.price_promo = round(product.price_promo, 2)
                product.is_sale = True
            if sale == "6":
                product.price_promo = 5
                product.is_sale = True
            if sale == "7":
                product.price_promo = 10
                product.is_sale = True
            if sale == "8":
                product.price_promo = 15
                product.is_sale = True
            if sale == "9":
                product.price_promo = 20
                product.is_sale = True
            product.save()
...
class UpdateProductView(PermissionRequiredMixin, UpdateView):
    ...
    # def form_valid(self, form):
    #     product = Products.objects.get(pk=form.instance.id)
    #     product.price = form.instance.price
    #     product.price_promo = form.instance.price_promo
    #     if form.instance.sale == 1:
    #         product.price_promo = form.instance.price * 0.9999
    #         product.price_promo = round(product.price_promo, 2)
    #         product.is_sale = True
    #     if form.instance.sale == 2:
    #         x = form.instance.price - (form.instance.price * 0.2)
    #         product.price_promo = round(x, 2)
    #         product.is_sale = True

    #     if form.instance.sale == 3:
    #         product.price_promo = form.instance.price * 0.7
    #         product.price_promo = round(product.price_promo, 2)
    #         product.is_sale = True
    #     if form.instance.sale == 4:
    #         product.price_promo = form.instance.price * 0.6
    #         product.price_promo = round(product.price_promo, 2)
    #         product.is_sale = True
    #     if form.instance.sale == 5:
    #         product.price_promo = form.instance.price * 0.5
    #         product.price_promo = round(product.price_promo, 2)
    #         product.is_sale = True
    #     if form.instance.sale == 6:
    #         product.price_promo = 5
    #         product.price_promo = round(product.price_promo, 2)
    #         product.is_sale = True
    #     if form.instance.sale == 7:
    #         product.price_promo = 10
    #         product.price_promo = round(product.price_promo, 2)
    #         product.is_sale = True
    #     if form.instance.sale == 8:
    #         product.price_promo = 15
    #         product.price_promo = round(product.price_promo, 2)
    #         product.is_sale = True
    #     if form.instance.sale == 9:
    #         product.price_promo = 20
    #         product.price_promo = round(product.price_promo, 2)
    #         product.is_sale = True
    #     product.name = form.instance.name
    #     product.shop_id = form.instance.shop_id
    #     product.user_id = form.instance.user_id
    #     product.mark_id = form.instance.mark_id
    #     product.category_id = form.instance.category_id
    #     product.factory_id = form.instance.factory_id
    #     product.material_id = form.instance.material_id
    #     product.qty = form.instance.qty
    #     product.is_active = form.instance.is_active
    #     product.is_new = form.instance.is_new
    #     product.info = form.instance.info
    #     product.sale = form.instance.sale

    #     if form.instance.price == form.instance.price_promo or form.instance.price_promo == None:
    #         product.is_promo = False
    #         product.price_promo = None
    #     else:
    #         product.is_promo = True
    #     if form.instance.sale == 0:
    #         product.is_sale = False
    #         product.price_promo = None
    #     redirect_url = super(UpdateProductView, self).form_valid(form)
    #     product.save()
    #     print(product)
    #     return redirect_url

    # def get_success_url(self, **kwargs):
    # return "/miktel/products_list/"
...
@method_decorator(login_required, name='dispatch')
class UpdateProductImageView(PermissionRequiredMixin, UpdateView):
...
class UpdateImageView(PermissionRequiredMixin, UpdateView):
    ...
    fields = '__all__'
    template_name_suffix = ('_update_form')
    success_url = "/miktel/images_list/"
...
class ProfileListView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        products_pagi = paginator.get_page(page)
        ctx = {'shops': shops, 'products': products_pagi}
        return TemplateResponse(request, "product_profile_list.html", ctx)
...
@method_decorator(login_required, name='dispatch')
class ImagesListView(PermissionRequiredMixin, View):
    permission_required = 'miktel.view_fotoproduct'

    def get(self, request):
        products = FotoProduct.objects.all()
        global page_records
        page = request.GET.get('page')
        paginator = Paginator(products, page_records)
        products_pagi = paginator.get_page(page)
        ctx = {'products': products_pagi}
        return TemplateResponse(request, "images_list.html", ctx)
...
class UpdateProfileView(PermissionRequiredMixin, UpdateView):
    permission_required = 'miktel.change_profile'
    raise_exception = True
    model = Profile
    fields = '__all__'
    template_name_suffix = ('_update_form')
    success_url = ('/miktel/profile_list/')


@method_decorator(login_required, name='dispatch')
class DelProfileView(PermissionRequiredMixin, DeleteView):
    permission_required = 'miktel.delete_profile'
    model = Profile
    fields = '__all__'
    template_name_suffix = ('_confirm_delete')
    success_url = ('/miktel/profile_list/')
...
class DelImageView(PermissionRequiredMixin, DeleteView):
    ...
    #     super().delete()
    #     image = FotoProduct.objects.get(image=self.object.image)
    #     os.remove(os.path.join(settings.MEDIA_ROOT, image.image))
    #     return f"/miktel/products_list/"
...
@method_decorator(login_required, name='dispatch')
class AddMaterialView(PermissionRequiredMixin, CreateView):
    permission_required = 'miktel.add_material'
    model = Material
    fields = '__all__'
    success_url = reverse_lazy("add_product")
