...
class MyUser(AbstractUser):
    ...
    sklep_dzisiaj = models.ForeignKey(
        "Sklep",
        null=True,
        blank=True,
        on_delete=models.CASCADE,
    ...
    class Meta:
        ...
        indexes = (models.Index(fields=['sklep_dzisiaj']), )
...
class Adres(models.Model):
    ...
    numerTelefonu = models.CharField(verbose_name="Numer telefonu",
                                     max_length=10)
    latitude = models.CharField(verbose_name="lititude",
                                max_length=32,
                                blank=True,
                                null=True)
    longitude = models.CharField(verbose_name="longitude",
                                 ...
                                 blank=True,
                                 null=True)
    google_maps = models.TextField(verbose_name="link do google maps",
                                   blank=True,
                                   null=True)
    ...
    def save(self, *args, **kwargs):
        self.slug = slugify(self.ulica) + "-" + slugify(self.miasto)
        super(Adres, self).save()
...
class Sklep(models.Model):
    ...
    class Meta:
        ...
        indexes = (models.Index(fields=['adres']), )
    ...
    def save(self, *args, **kwargs):
        self.slug = slugify(self.nazwa) + "-" + slugify(self.adres.slug)
        super(Sklep, self).save()

    def get_absolute_url(self):
        return reverse("contact_page", kwargs={
            "slug": self.slug,
        })
    ...
    def serwis_reklamacja(self):
        serwis_reklamacja = DodajSerwis.objects.filter(status=7).filter(
            sklep=self)
        return serwis_reklamacja.count()
    ...
    def __str__(self):
        return slugify(str(self.nazwa)) + "-" + str(self.adres.miasto_slug)
...
class UmowaKomisowaNew(models.Model):
    ...
    class Meta:
        ...
        indexes = (models.Index(fields=['phones', 'sprzedana']), )
...
class Telefon(models.Model):
    ...
    cena_promo = models.DecimalField(blank=True,
                                     null=True,
                                     max_digits=6,
                                     decimal_places=2)
    ...
    class Meta:
        ...
        indexes = (models.Index(
            fields=['sklep', 'dostepny', 'marka', 'nazwa']), )
    ...
    def is_in_promo(self):
        if self.cena_sprzed != self.cena_promo:
            self.in_promo = True
            self.save()
...
class Marka(models.Model):
    ...
    def foto_mini(self):
        foto_mini = FotoProduct.objects.filter(mark_id=self.id).first()
        if foto_mini != None:
            return foto_mini

    def save(self, *args, **kwargs):
        self.slug = slugify(self.nazwa)
        super(Marka, self).save()

    @property
    def mark_counter(self):
        products_count = Products.objects.filter(mark_id=self).count()
        return products_count

    def get_absolute_url(self):
        return reverse("auto-keys-by-mark", kwargs={
            "mark": self.slug,
        })
...
class PremiaJob(models.Model):
    ...
    class Meta:
        ...
        indexes = (models.Index(fields=['pracownik', 'sklep']), )
...
class DodajSerwis(models.Model):
    ...
    class Meta:
        ...
        indexes = (models.Index(fields=['sklep', 'status']), )
...
class Czesc(models.Model):
    ...
    class Meta:
        ...
        indexes = (models.Index(fields=['marka', 'typ', 'nazwa', 'sklep']), )
    ...
    def save(self, *args, **kwargs):
        self.slug = slugify(self.marka.nazwa) + "-" + slugify(
            self.nazwa) + "-" + slugify(
                self.typ.nazwa) + "-dostepny-w-miktel-" + slugify(
                    self.sklep.adres.slug)
    ...
    def __str__(self):
        return str(self.id) + " " + str(self.marka.nazwa) + " " + str(
            self.nazwa) + " " + str(self.typ) + " " + str(
                self.get_kolor_display()) + " " + str(
                    self.cena_sprzed) + " " + str(self.sklep)
...
class Products(models.Model):
    ...
    sale = models.IntegerField(verbose_name="Wyprzedaż",
                               choices=SALE_SORTED,
                               default=0)
    ...
    class Meta:
        ...
        indexes = (models.Index(fields=['category_id', 'name', 'shop_id']), )
    ...
    @property
    def prod_count_by_mark(self):
        prod_count_by_mark = Products.objects.filter(mark_id=self.mark_id)
        return prod_count_by_mark

    def save(self, *args, **kwargs):
        self.slug = slugify(self.name)
        self.price = round(self.price, 2)
        if self.sale == 1:
            self.price_promo = self.price * 0.9
            self.price_promo = round(self.price_promo, 2)
            self.is_sale = True
        if self.sale == 2:
            self.price_promo = self.price * 0.8
            self.price_promo = round(self.price_promo, 2)
            self.is_sale = True
        if self.sale == 3:
            self.price_promo = self.price * 0.7
            self.price_promo = round(self.price_promo, 2)
            self.is_sale = True
        if self.sale == 4:
            self.price_promo = self.price * 0.6
            self.price_promo = round(self.price_promo, 2)
            self.is_sale = True
        if self.sale == 5:
            self.price_promo = self.price * 0.5
            self.price_promo = round(self.price_promo, 2)
            self.is_sale = True
        if self.sale == 6:
            self.price_promo = 5
        ...
        if self.sale == 9:
            ...
            self.price_promo = round(self.price_promo, 2)
            self.is_sale = True
        if self.price == self.price_promo or self.price_promo == None or self.sale == 0:
            self.is_promo = False
...
class Factory(models.Model):
    ...
    def foto_mini_akc(self):
        foto_mini = FotoProduct.objects.filter(factory_id=self.id).first()
        if foto_mini != None:
            return foto_mini
...
class Categorys(models.Model):
    ...
    def counter_items(self):
        products_count = Products.objects.filter(category_id=self).count()
        return products_count
...
class FotoProduct(models.Model):
    ...
    #     self.image.storage.delete(self.image.image)
    #     super().delete()
...
class Articles(models.Model):
    ...
    site_id = models.IntegerField(verbose_name="Strona",
                                  choices=STRONY,
                                  null=True,
                                  blank=True)
    ...
    def __str__(self):
        return str(self.date) + ", " + str(self.title) + ", " + str(
            self.site_id)
